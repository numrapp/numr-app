...
export default function OfferteEditPage() {
  ...
  const [form, setForm] = useState({ offerte_number: '', offerte_date: '', valid_until: '', description: '' });

  useEffect(() => {
    offerteService.getById(Number(id)).then((off: any) => {
      setOfferte(off);
      setForm({
        offerte_number: off.offerte_number,
        offerte_date: off.offerte_date,
        valid_until: off.valid_until || '',
        description: off.description || '',
      });
      const offItems = off.items && off.items.length > 0 ? off.items : [{ description: off.description || '', quantity: 1, unit_price: off.subtotal || 0, btw_rate: 21 }];
      setItems(offItems);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await offerteService.update(Number(id), {
        ...form,
        items,
  ...
  if (!offerte) return <div className="h-full flex items-center justify-center text-gray-400">Offerte niet gevonden</div>;
      ...
      <div className="page-scroll px-6 pb-6">
        <div className="space-y-4 max-w-md mx-auto">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label-send">Offertenummer</label>
              <input type="text" value={form.offerte_number} onChange={e => setForm(p => ({...p, offerte_number: e.target.value}))} className="input-send notranslate" />
            </div>
            <div>
              <label className="label-send">Datum</label>
              <input type="date" value={form.offerte_date} onChange={e => setForm(p => ({...p, offerte_date: e.target.value}))} className="input-send" />
            </div>
          </div>

          <div>
            <label className="label-send">Geldig tot</label>
            <input type="date" value={form.valid_until} onChange={e => setForm(p => ({...p, valid_until: e.target.value}))} className="input-send" />
          </div>
