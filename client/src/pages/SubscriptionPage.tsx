...
export default function SubscriptionPage() {
  ...
  const handlePurchase = async (type: 'monthly' | 'yearly' | 'trial') => {
    ...
    try {
      if (type === 'trial') {
        await api.post('/auth/subscribe', { type: 'trial' });
        await refreshUser();
        navigate('/');
        return;
      }
        ...
        <div className="max-w-sm mx-auto">
          ...
          <div className="space-y-3">
            ...
            <button onClick={() => handlePurchase('yearly')} disabled={!!loading}
              ...
              {loading === 'yearly' && <div className="absolute inset-0 bg-white/60 flex items-center justify-center rounded-3xl"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-brand" /></div>}
            </button>

            <button onClick={() => handlePurchase('monthly')} disabled={!!loading}
              className="w-full p-5 rounded-3xl border border-gray-200 bg-white transition-all active:scale-[0.97] relative text-left">
              <div className="flex items-center justify-between">
                <div>
              ...
              {loading === 'monthly' && <div className="absolute inset-0 bg-white/60 flex items-center justify-center rounded-3xl"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-brand" /></div>}
            </button>
          </div>
          ...
          <div className="text-center mt-5 space-y-2">
            ...
            <button onClick={handleRestore} disabled={restoring}
              className="flex items-center justify-center gap-1.5 mx-auto text-sm font-bold text-gray-400 hover:text-dark transition-colors">
