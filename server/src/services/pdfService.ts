...
export function generateInvoicePDF(data: InvoiceData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica').fillColor(md).text('Betalingstermijn', L + 5, y + 57); doc.font('Helvetica-Bold').fillColor(dk).text('14 dagen', L + 85, y + 57);

    doc.font('Helvetica').fontSize(9).fillColor(md);
    doc.text('Factuurnummer', rx, y + 18); doc.font('Helvetica-Bold').fillColor(dk).text(data.invoiceNumber, rx + 90, y + 18);
    doc.font('Helvetica').fillColor(md).text('Factuurdatum', rx, y + 33); doc.font('Helvetica-Bold').fillColor(dk).text(dt(data.invoiceDate), rx + 90, y + 33);
